...
逻辑冲突严格意义上说不算是 *Git* 的冲突。但是这里还是讲一讲吧。有不限于以下情况：开发过程中公共的**头文件**、**函数名**和其他开发者冲突了，为了解决这些冲突你修改了这些名称，但是原有名称在你自己那些代码里引用了... 这个时候从文件角度上说 *Git* 不认为还有冲突，但是你的代码 编译/执行 会出现错误。这种冲突问题就是 **逻辑冲突**。
